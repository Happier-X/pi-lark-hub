...
async function main() {
 ...
 const hub=await startHubServer({host:config.host,port:config.port,feishu:transport,allowedOpenIds:config.allowedOpenIds,hubConfig:config,
  onNativeRuntime:async(candidate,creds,server)=>{const handlers={onMessage:async(input:{text:string;openId?:string;replyToMessageId?:string})=>{const r=await server.handleInboundMessage(input);return{ok:r.ok,reply:r.reply}},onApprovalAction:async(input:{requestId:string;decision:"approve"|"reject";openId?:string})=>{const r=await server.handleInboundApproval(input);return{ok:r.ok,reply:r.reply}},replyToUser:async(text:string)=>{await candidate.send({body:text})}};const ws=new NativeFeishuWsInbound(creds,handlers,{log:console.log});await ws.start();const stop=()=>ws.stop();inboundStop=stop;return stop},
  onReady:async server=>{if(!credentials)return;const handlers={onMessage:async(input:{text:string;openId?:string;replyToMessageId?:string})=>{const r=await server.handleInboundMessage(input);return{ok:r.ok,reply:r.reply}},onApprovalAction:async(input:{requestId:string;decision:"approve"|"reject";openId?:string})=>{const r=await server.handleInboundApproval(input);return{ok:r.ok,reply:r.reply}},replyToUser:async(text:string)=>{await server.feishu.send({body:text})}};const ws=new NativeFeishuWsInbound(credentials,handlers,{log:console.log});await ws.start();const stop=()=>ws.stop();inboundStop=stop;return stop}
 });
