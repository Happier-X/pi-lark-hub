...
- 出站 notify 历史可查（`GET /notifications` 的 records）；failed 可显式 `重试`/`POST /control/notify-retry`；审批重试不新建 requestId；不做自动批次续发。
- 未决审批与消息绑定可轻量持久化到 `state.json`（原子写 + schemaVersion）；启动恢复 pending 超时；不落 secret；损坏则空状态启动。
