...
将飞书操作收敛为 /lark 与 /lark reset，删除短码配对、旧命令、console 和 lark-cli 兼容路径，保留事务式官方扫码、可信主人校验与原生 OpenAPI/WebSocket。
...
## Session 12: 飞书出站卡片 Markdown 渲染

**Date**: 2026-07-21
**Task**: 飞书出站卡片 Markdown 渲染
**Branch**: `main`

### Summary

出站通知优先 interactive 卡片 Markdown，失败降级 text；单条截断不拆多条；补单测与出站合约文档。
...
## Session 13: 路线图逐项落地：P0 到 e2e/CI

**Date**: 2026-07-22
**Task**: 路线图逐项落地：P0 到 e2e/CI
**Branch**: `main`

### Summary

按优化路线图逐项实现并归档：协议运行时解码、notify 幂等与 ack 队列、重连 piId 与 approval_result_ack、审批卡片按钮回调、/lark status、控制面 token/限流、队列查看取消、通知历史与显式重试、审批绑定持久化、Hub e2e 与发布检查；另含长卡片完整展示与路线图文档。
